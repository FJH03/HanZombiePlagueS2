...
public class HZPHumanModelMenu
{
    ...
    public IMenuAPI? OpenHumanModelMenu(IPlayer player)
    {
        ...
        var models = _helpers.GetEnabledHumanModels(humanCFG);
        if (models.Count == 0)
        {
        ...
        defaultButton.Click += async (_, args) =>
        {
            ...
            _core.Scheduler.NextTick(() =>
            {
                ...
                ApplyModelImmediatelyIfPossible(clicker, cfg, humanCFG);
    ...
    private void ApplyModelImmediatelyIfPossible(IPlayer player, HZPMainCFG cfg, HZPHumanClassCFG humanCFG)
    {
        if (player == null || !player.IsValid)
            return;
